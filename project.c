...
void words_for_third_list(int m,listT *head2,char *check_word,listT *head3) {  /*m=words' length*/
	listT *curr;
	char *buffer,*buffer1;
	int i,j,counter=0;
...
void add_word_same_len(int len,char *filename,listT *head2) {
	...
	if(head2->lunion.counter<2) {
		printf("Game has less than 2 words, so it cannot be continued\n");
		head2=eksodos();
		exit(1);
	}
}
...
int processing_arguments(int a,char *b[]) {
...
int main(int argc, char *argv[]) {     /*argv[1]=length of the 2 words argv[2]=steps argv[3]=dictionary file*/
	
	
	
	listT *head1; /*list1: words in the path each time*/
	listT *head2; /*list2: words of given length*/
	listT *head3; /*list3: available words to be selected for the next step*/
	int mikos,check,checking;
	int vimata;
	int r_num1,r_num2; /*r_num1=starting word & r_num2=target word*/
	char *s_word,*t_word;
	char *choosen_word; /*the word that is chosen by the player for the next step*/
	char formatstr[15];
	
	srand(time(NULL));
	
	check=processing_arguments(argc,&argv[0]);
	mikos=atoi(argv[1]);
	if(check==0) {
		return (0);
	}
